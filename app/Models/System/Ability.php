...
class Ability extends Model
{
    ...
    {
        return $this
            ->belongsToMany(Page::class, 'system_abilities_pages')
            ->withPivot('role', 'slug')
            ->orderBy('system_pages._lft');
    }
     ...
     * @return void
     */
    public function attachPage($page)
    {
        $page = Page::firstWhere('slug', $page);

        if (! $page) {
            return;
        }

        $this->pages()->syncWithoutDetaching([
            optional($page)->id => [
                'role' => $this->slug,
     ...
     * @return void
     */
    public function attachPermission($page, $permission)
    {
        if (! $page || ! $permission) {
            return;
        }
